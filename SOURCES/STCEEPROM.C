...
U8 STC_EE_Write(U16 Addr, U8 *Buf, U16 Num)
{
    ...
    // 擦除扇区2
    Sector_Erase(0x200); // 擦除第2扇区
    // 复制第1扇区数据到第2扇区备份
    for (i = 0; i < 0x200; i++)
    {
        tmp = Byte_Read(i);           // 读EEPROM的值
        Byte_Program(i + 0x200, tmp); // 写入 EEPROM
    }
    // 擦除扇区1
    ...
    for (i = 0; i < Addr; i++)
    {
        tmp = Byte_Read(i + 0x200); // 读EEPROM的值
...
void PWM_Write_value(U16 dat) // 写入当前的亮度值
{
    U8 Buf[2];
    Buf[0] = (dat >> 8) & 0X00FF; // 
    Buf[1] = dat & 0X00FF;        //  
    STC_EE_Write(0x06, Buf, 2);   // byte 00  01
}

U16 PWM_Read_value(void) // 读出关灯前的亮度值
{
    U16 dat_H, dat_L, DAT;
    U8 Buf[2];
    STC_EE_Read(0x06, Buf, 2);      // byte 00  01
    dat_H = (Buf[0] << 8) & 0Xff00; //  
    dat_L = Buf[1] & 0X00ff;        //  
    DAT = dat_H | dat_L;
    return DAT;
}
